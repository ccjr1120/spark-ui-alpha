...
export default ({ view, date, openType, calendars, onChange }) => {
  ...
  const opType = view === 'day' ? 'month' : view === 'year' ? '' : 'year';
  const [hoverDate, setHoverDate] = useState(null);
  const [selectDate, setSelectDate] = useState(date);
  const [selectStartDate, selectEndDate] = useMemo(() => selectDate, [selectDate]);
  ...
  const getDateOpLabel = (index) => {
    ...
    return `${opType === 'month' ? chsMonth[newDate.month()] : ''}  ${newDate.year()}年`;
  };
  /**
